/**
 * Extractor de órdenes de compra desde archivos PDF.
 * Usa pdf-parse para extraer el texto y Claude API para estructurar los datos.
 */
...
export async function extractFromPDF(fileBuffer: Buffer): Promise<OrdenCompra> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Error en Claude API (${response.status}): ${text}`);
  }
  ...
  try {
    ...
    throw new Error(`Error al parsear la respuesta de Claude: ${err instanceof Error ? err.message : String(err)}\n\nRespuesta recibida: ${content.substring(0, 500)}`);
  }

  // Normalizar items (asegurar que tengan IDs únicos)
